...
class FlightCotrollerTest {
   ...
   /* @Test
    void get_All_Flights_In_Between() throws ParseException {
        ...
        assertEquals(1, flightController.getAllFlightsBetween(airPortOrigin.getCode()
                , airPortDestination.getCode(),"05 20, 2020").size());
    }*/
    @Test
    void getAirlineByDepartingAirportCode() throws IOException {
        when(flightRepository.findAllByOriginCode(airPortOrigin.getCode())).
                thenReturn(Arrays.asList(flight));
        assertEquals(Arrays.asList(airline), flightService.findDepartingAirlines(airPortOrigin.getCode()));
    }
}